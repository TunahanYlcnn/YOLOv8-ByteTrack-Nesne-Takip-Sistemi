...
def ious(atlbrs, btlbrs):
    ...
    :param atlbrs: array-like of shape (N,4), each [x1, y1, x2, y2]
    :param btlbrs: array-like of shape (M,4), each [x1, y1, x2, y2]
    :return: IoU matrix of shape (N, M)
    """
    # Convert to NumPy arrays
    boxes1 = np.asarray(atlbrs, dtype=np.float32)
    boxes2 = np.asarray(btlbrs, dtype=np.float32)
    ...
    for i in range(N):
        ...
        for j in range(M):
            ...
            wi = max(0.0, xi2 - xi1 + 1)
            hi = max(0.0, yi2 - yi1 + 1)
            inter_area = wi * hi
